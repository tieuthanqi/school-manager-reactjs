...
const ContactBookForm = () => {
  ...
  useEffect(() => {
    const loadContactBookEdit = async () => {
      ...
      if (id !== -1) {
        await dispatch(getContactBookById(`/contact-book/teacher/${id}`));
  ...
  const handleSubmit = (values) => {
    ...
    console.log(values);
    if (id === -1) {
      dispatch(addContactBookByStudent('/contact-book/teacher',params, history));
      console.log("add");
    } else {
      params.id = id;
      console.log(params);
      dispatch(updateContactBook('/contact-book/teacher', params, history));
      console.log("update");
    }
